...
module OmniAuth
  module Strategies
    ...
    class OAuth2
      ...
      def client
        OmniAuth.logger.send :debug, "OAuth2::client >>>>>>>>>>"
        OmniAuth.logger.send :debug, options.client_id
        if options.client_secret?
          OmniAuth.logger.send :debug, options.client_secret
          # puts options.client_secret
        else
          OmniAuth.logger.send :debug, "no secret"
          # puts "no secret"
        end
        OmniAuth.logger.send :debug, options
        # OmniAuth.logger.send :debug, options.client_options

        _client = ::OAuth2::Client.new(options.client_id, options.client_secret, deep_symbolize(options.client_options))
        OmniAuth.logger.send :debug, "OAuth2::client <<<<<<<<<<"
        _client
      end
      ...
      def authorize_params
        OmniAuth.logger.send :debug, "OAuth2::authorize_params >>>>>>>>>>"
      ...
      def pkce_token_params
        OmniAuth.logger.send :debug, "OAuth2::pkce_token_params >>>>>>>>>>"
        return {} unless options.pkce
      ...
      def deep_symbolize(options)
        OmniAuth.logger.send :debug, "OAuth2::deep_symbolize >>>>>>>>>>"
        OmniAuth.logger.send :debug, "OAuth2::deep_symbolize ----------"
      ...
      def options_for(option)
        hash = {}
        OmniAuth.logger.send :debug, "OAuth2::options_for >>>>>>>>>>"
